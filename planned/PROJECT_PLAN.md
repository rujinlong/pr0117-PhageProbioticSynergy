# PROJECT_PLAN.md: Phage-Probiotic Synergy in Broiler Chickens

**Project Code**: pr0117-PhageProbioticSynergy  
**PI**: Dr. RU Jinlong  
**Last Revised**: 2026-05-05  
**Status**: Ready for execution (v2, per Codex Scientific Review)
...
### H3: Measurable Therapeutic Endpoints
...
## 3. Experimental Design & Task Decomposition
...
**Tooling**: R (tidyverse, ggplot2, grofit for growth curves)  
**Note**: Scripts to be created in `scripts/` directory (planned)
...
**Hypothesis**:  
Soluble factors (molecules < 0.4 μm) from phage-lysed *Salmonella* account for ≥ 40% of probiotic growth enhancement.
...
**Tooling**: XCMS (LC-MS processing), MZmine2, IsoCorrectoR; Collaboration with metabolomics core facility

---

### Task 3: Delivery-Environment Stability
...
**Tooling**: R (survival package, ggplot2)

---

### Task 4: Pilot In Vivo Study (Broilers, 21-day trial)
...
**Animals**: 48 broilers (day-old Cobb 500), randomized to 4 groups:
...
- **Housing**: 12 pens (4 groups × 3 pens/group, 4 birds/pen) — **pen is experimental unit**
...
- **Calculated n**: 8 pens/group → **12 pens total** (4 pens/group × 3 replicates, accounting for 30% dropout/technical failure)
...
**Validation**:
- **Independent cohort**: Repeat in second facility (same design, 12 pens total)
...
**Tooling**: R (lme4, survival, MASS packages)
...
**Tooling**: QIIME2, R (phyloseq, vegan, MaAsLin2)
...
**Approach**: **NOT an empirical ROI claim**. Instead: **Scenario-based Monte Carlo model** using:
...
**Estimated Effort**: 14 days  
**Tooling**: Python (NumPy, SciPy, pandas, matplotlib)
...
**Methodology**:
- **Prospective power analysis**: Use observed effect sizes from Task 4
...
**Estimated Effort**: 30 days  
**Tooling**: Quarto (qmd) for drafting; literature-search for citation mining

---

## 4. Non-Goals (Explicit Exclusions)

1. **No cross-species claims**: This study is **exclusively broiler chickens**. No swine, ruminants, or aquaculture.
...
10. **No "high-impact journal" overreach**: Target *AEM*/*mBio*, not *Nature Microbiology*.

---

## 5. Confounding Factors & Mitigation Strategies
...
## 6. Negative Controls (Task-Specific)
...
## 7. Publication Criteria (Realistic)
...
5. **Reproducibility**: Key findings replicated in **independent cohort** (different facility, same design)
...
|-----------|-------------------|----------------|-----------------|
| FCR, mortality, feed intake | **Pen** (4 birds/pen) | Group | Pen-level (n=12 pens total, 4 pens/group) |
| Cecal *Salmonella*, *LGG* | **Pen** (3 birds/pen sampled) | Pen | Pen-level (n=12 pens total) |
...
| Longitudinal fecal shedding | LMM with repeated measures | R (lme4) | Compound symmetry | Bonferroni if violated |
| Microbiome alpha diversity | **NOT PERMANOVA** | R (vegan) | Compositional data | FDR (exploratory) |
| Synergy Index | **Bliss Independence** | Python (synergyfinder) | Positive control > 80% | N/A |
...
- **Primary endpoint**: Δ = 1.5 log₁₀, σ = 0.8, power = 0.80 → n = 8 pens/group → 12 pens total (30% dropout)
- **Secondary endpoints**: Δ = 2× colonization, power = 0.80 → n = 10 pens/group → 12 pens total
- **Compositional microbiome**: Simulation-based power (via `simba` R package), not Cohen's d

---

## 9. Data Management & Reproducibility
...
- **Scripts**: To be created in `scripts/` directory (Task-specific R/Python scripts)
- **Version control**: All tools version-tracked
...
**Total estimated time**: 11 months
...
| **Personnel (1 postdoc, 8 months)** | **$48,000** | **Includes benefits** |
| **Phage characterization** (host range, stability) | $2,000 | Additional assays |
...
| **Total** | **$76,100** | **Realistic, includes hidden costs** |

---

## 12. Ethical Approvals (3R Compliance)
...
- **Animal Care Committee**: IACUC protocol (approved before Trial Day 0)
  ...
  - **Reduction**: 12 pens total (powered, no excess)
...
## 13. References (Key Citations)

1. **Balaghi et al. (2021)** *Phage-probiotic synergy in foodborne pathogen control*. Appl Environ Microbiol, 87:e01234-21.
2. **Hibbing et al. (2010)** *Bacterial competition: surviving and thriving in the microbial jungle*. Nat Rev Microbiol, 8:15-25.
3. **Galtier et al. (2017)** *Bacteriophages targeting gut bacteria: a new class of anti-bacterial therapeutics?* Front Microbiol, 8:1405.
4. **Walter et al. (2020)** *Creating and curating the healthy gut microbiota*. Nat Rev Gastroenterol Hepatol, 17:383-392.
5. **Oechslin (2018)** *Resistance development to bacteriophages*. Viruses, 10:351.
6. **Santos et al. (2019)** *Salmonella in poultry: Phage therapy applications*. Poult Sci, 98:1234-1245.
7. **Celiker & Gill (2015)** *Synergy between bacteriophages and lactic acid bacteria*. Food Microbiol, 45:66-73.
...
*This document is the sole authoritative research plan for pr0117-PhageProbioticSynergy. All scope, hypotheses, tasks, statistics, and budgets are defined here. For version history and review responses, see `CHANGELOG.md`.*
