...
This module focuses on mining publicly available datasets relevant to phage-probiotic synergy in animal health:
...
Search for metagenomic and metatranscriptomic datasets from animal gut samples.
...
```{r phage-genome-download}
#| label: phage-genome-download
#| eval: false
...
1. **Complete NotebookLM research** import (check status with `nlm research status phage-synergy`)
2. **Run SRA searches** (enable evaluation in Section 2.1 when API key ready)
